...
export function findProtectedRegions(text: string): Region[] {
	...
	const filenameRe =
		/\b[A-Za-z][\w./-]*\.(?:md|txt|json|toml|yaml|yml|ts|js|tsx|jsx|rs|go|py|rb|java|c|cpp|h|sh|bash|zsh|html|css|scss|xml|sql|lock|log|env|gitignore|dockerignore|editorconfig|prettierrc|eslintrc)\b/g;
...
export function parseSuggestion(s: string): string {
	if (s.startsWith("Replace with: ")) {
		return s
			.slice("Replace with: ".length)
			.replace(/^[\u201C"\u201D]+|[\u201C"\u201D]+$/g, "");
	}
	if (s.startsWith("Remove")) return "";
	return s;
}
...
export function applyFixes(
	...
	for (const lint of sorted) {
		...
		const original = result.substring(lint.span.char_start, lint.span.char_end);
		result =
...
export const HarperSpellCheck = async ({ client }: any) => {
	...
	return {
		"chat.message": async (input: any, output: any) => {
			if (!PRIMARY_AGENTS.has(input?.agent)) return;
